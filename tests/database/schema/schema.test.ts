...
describe("Schema Class", () => {
  describe("Basic Schema Creation", () => {
    ...
    it("should handle constructor shortcuts", () => {
      ...
      const metadataColumn = tableSchema.columns.find(c => c.name === "metadata");
      ...
      const createdAtColumn = tableSchema.columns.find(c => c.name === "created_at");
  ...
  describe("Advanced Field Definitions", () => {
    it("should handle references using ref shortcut", () => {
      ...
      const userIdColumn = tableSchema.columns.find(c => c.name === "user_id");
      expect(userIdColumn?.references).toEqual({
        table: "users",
        column: "id"
      });
      
      const roleIdColumn = tableSchema.columns.find(c => c.name === "role_id");
    ...
    it("should handle check constraints", () => {
      ...
      expect(statusColumn?.check).toBe("status IN ('active', 'inactive', 'suspended')");
    ...
    it("should handle Date.now default values", () => {
      ...
      const updatedAtColumn = tableSchema.columns.find(c => c.name === "updated_at");
    ...
    it("should handle function default values", () => {
      ...
      const randomNumberColumn = tableSchema.columns.find(c => c.name === "random_number");
  ...
  describe("Type Mapping", () => {
    it("should map constructor types to SQL types correctly", () => {
      ...
      expect(tableSchema.columns.find(c => c.name === "id")?.type).toBe("TEXT");
      expect(tableSchema.columns.find(c => c.name === "count")?.type).toBe("INTEGER");
      expect(tableSchema.columns.find(c => c.name === "active")?.type).toBe("BOOLEAN");
      expect(tableSchema.columns.find(c => c.name === "created_at")?.type).toBe("DATETIME");
      expect(tableSchema.columns.find(c => c.name === "metadata")?.type).toBe("TEXT");
      expect(tableSchema.columns.find(c => c.name === "tags")?.type).toBe("TEXT");
    });
    ...
    it("should handle string types directly", () => {
      ...
      expect(tableSchema.columns.find(c => c.name === "id")?.type).toBe("TEXT");
      expect(tableSchema.columns.find(c => c.name === "priority")?.type).toBe("INTEGER");
      expect(tableSchema.columns.find(c => c.name === "enabled")?.type).toBe("BOOLEAN");
      expect(tableSchema.columns.find(c => c.name === "expires_at")?.type).toBe("DATETIME");
    });
  });
  ...
  describe("Complex Schema Structures", () => {
    it("should handle nested objects correctly", () => {
      ...
      const profileColumn = tableSchema.columns.find(c => c.name === "profile");
      expect(profileColumn?.type).toBe("TEXT");
      expect(profileColumn?.defaultValue).toBe("{}");
      
      const preferencesColumn = tableSchema.columns.find(c => c.name === "preferences");
    ...
    it("should handle arrays correctly", () => {
      ...
      const numbersColumn = tableSchema.columns.find(c => c.name === "numbers");
      expect(numbersColumn?.type).toBe("TEXT");
      expect(numbersColumn?.defaultValue).toBe("[]");
      
      const complexColumn = tableSchema.columns.find(c => c.name === "complex");
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle undefined values gracefully", () => {
      ...
      const optionalColumn = tableSchema.columns.find(c => c.name === "optional_field");
      ...
      const withDefaultColumn = tableSchema.columns.find(c => c.name === "with_default");
      expect(withDefaultColumn?.defaultValue).toBe("default_value");
      
      const withCheckColumn = tableSchema.columns.find(c => c.name === "with_check");
  ...
  describe("Real-World Schema Examples", () => {
    it("should handle a complete user profile schema", () => {
      const userProfileSchema = new Schema({
        ...
        indexes: [
          { name: "idx_user_profiles_user_id", columns: ["user_id"], unique: true },
      ...
      expect(userIdColumn?.references).toEqual({ table: "users", column: "id" });
    ...
    it("should handle OAuth client schema", () => {
      ...
      const clientIdColumn = tableSchema.columns.find(c => c.name === "client_id");
      expect(clientIdColumn?.notNull).toBe(true);
      expect(clientIdColumn?.unique).toBe(true);
      
      const clientNameColumn = tableSchema.columns.find(c => c.name === "client_name");
      ...
      const isPublicColumn = tableSchema.columns.find(c => c.name === "is_public");
