...
import { OAuthGrantType, OAuthResponseType, PKCEMethod, BiometricType, MFAType, OAuthErrorType } from '../src/types/oauth';
...
describe('OAuth 2.0 Service Tests', () => {
  ...
  beforeAll(async () => {
    ...
    oauthMiddleware = new OAuthSecurityMiddleware(oauthService, securityService, jwtService);
  });
  ...
  describe('OAuth Client Management', () => {
    ...
    test('should authenticate client with valid credentials', async () => {
      const client = await oauthService.authenticateClient('test-client-id', 'test-secret');
      expect(client).toBeDefined();
      expect(client!.client_id).toBe('test-client-id');
    });

    test('should reject authentication with invalid credentials', async () => {
      const client = await oauthService.authenticateClient('test-client-id', 'wrong-secret');
  ...
  describe('Enhanced User Features', () => {
    ...
    test('should register biometric credentials', async () => {
      ...
      expect(bioResult.credential!.biometric_type).toBe(BiometricType.FINGERPRINT);
    });
  ...
  describe('OAuth Security Middleware', () => {
    test('should validate authorization request', async () => {
      ...
      const client = await oauthService.findClientByClientId('test-client-id');
      const validation = await oauthMiddleware.validateAuthorizationRequest(request, client || undefined);
